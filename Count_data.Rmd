...
```{r}
pois <- glm(cases ~ age +city + log(pop), data=final, 
...
The Poisson regression model is fitted using the numeric versions of age and city, and population size as the exposure. Note that pop is an exposure, and not an offset. The offset statement is included in Stata mainly for consistency with other software packages. It is simpler to use the exposure statement in Stata.
